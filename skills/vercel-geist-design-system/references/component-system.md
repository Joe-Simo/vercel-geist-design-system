...
- Navigation, layout, and disclosure: Tabs, Pagination, Collapse, Drawer, Sheet, Modal, Grid, Show More, Scroller, Table. In this mapping, Grid means the official visible-guide/cell component only when that visible component is explicitly intended; ordinary page columns and card layouts should use CSS grid/flex without visible guides.
...
- If the requirement is "full-page grid", "Vercel-like grid", "dashboard grid", or "feature grid", default to a functional content grid: real cells, rows, columns, alignment, and borders. Do not use the official Grid component or visible guide lines unless the wording explicitly asks for visible guides/cells or the source design unmistakably shows them.
