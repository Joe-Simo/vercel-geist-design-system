...
- Navigation, layout, and disclosure: Tabs, Pagination, Collapse, Drawer, Sheet, Modal, Grid, Show More, Scroller, Table. In this mapping, Grid means the official visible-guide/cell component when the user asks for Vercel.com-like grid art, visible guide/cell layout, a full-page Grid treatment, or the supplied design clearly intends that exact surface; ordinary page columns, card layouts, and solid bordered content grids should use CSS grid/flex without visible guide backgrounds.
...
- Custom primitive aliases: IconButton -> Button; List -> Table, Entity, Description, Scroller, or Show More; Card/Panel/Surface -> Material, then Context Card, Relative Time Card, Project Banner, Entity, Description, or Table by content; AppShell -> CSS grid/flex plus Menu, Tabs, Drawer, Sheet, Button, Avatar, Badge, and Status Dot as used. Use Geist Grid when visible cell/guide layout is explicitly requested, the user asks for a Vercel.com-like grid/art treatment, it is present in a supplied design, or the official Grid component is the intended UI surface. Scope visible guide backgrounds to the specific visible-grid surface; do not apply them to the whole page, ordinary panels, or unrelated content grids unless full-page visible grid art is explicitly requested. Do not add graph-paper, blueprint, or repeating-gradient decoration behind unrelated UI. These aliases are not escape hatches; they inherit applicable Best Practices from every adjacent official component.
...
- If the requirement is "full-page grid", "Vercel-like grid", "Vercel.com grid", "grid art", "dashboard grid", or "feature grid", first determine whether the user means ordinary responsive layout or the visible Vercel/Grid aesthetic, then identify the exact surface. Explicit Vercel.com references, grid art, guide lines, visible cells, or supplied Vercel-like screenshots mean an intentional visible cell-and-guide surface only where those lines are requested or shown. Otherwise default to a functional content grid with solid backgrounds, real cells, rows, columns, alignment, and borders.
